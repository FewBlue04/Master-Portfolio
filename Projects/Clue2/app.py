...
from engine.cards import CARD_TYPE, ROOMS, SUSPECTS, WEAPONS
...
class ClueApp(tk.Tk):
    ...
    def _show_setup(self):
        ...
        tk.Label(hero, text="CLUE", bg=BG_DARK, fg=GOLD, font=("Georgia", 34, "bold")).pack(pady=(0, 4))
        tk.Label(hero, text="Luxury Noir Edition", bg=BG_DARK, fg=TEXT_DIM, font=("Georgia", 11, "italic")).pack(pady=(0, 20))

        frame = tk.Frame(hero, bg=BG_PANEL, padx=30, pady=24, highlightbackground=GOLD, highlightthickness=1)
        frame.pack()

        tk.Label(frame, text="Your Name", bg=BG_PANEL, fg=TEXT_MAIN, font=("Georgia", 12)).grid(row=0, column=0, sticky="w", pady=8)
        tk.Entry(frame, textvariable=self.setup_name_var, bg=BG_CARD, fg=TEXT_MAIN, insertbackground=GOLD, font=("Georgia", 12), width=18, relief="flat", bd=4).grid(row=0, column=1, padx=(12, 0), pady=8)

        tk.Label(frame, text="Bot Count", bg=BG_PANEL, fg=TEXT_MAIN, font=("Georgia", 12)).grid(row=1, column=0, sticky="w", pady=8)
        tk.Spinbox(frame, from_=1, to=5, textvariable=self.setup_bots_var, bg=BG_CARD, fg=TEXT_MAIN, buttonbackground=BG_PANEL, font=("Georgia", 12), width=5, relief="flat").grid(row=1, column=1, sticky="w", padx=(12, 0), pady=8)

        tk.Button(hero, text="Begin Investigation", bg=GOLD, fg=BG_DARK, activebackground=GOLD_LIGHT, font=("Georgia", 13, "bold"), relief="flat", padx=18, pady=10, command=self._start_new_game).pack(pady=(18, 0))
    ...
    def _panel(self, parent, title):
        return tk.LabelFrame(parent, text=f" {title} ", bg=BG_PANEL, fg=GOLD, font=("Georgia", 10, "bold"), bd=1, relief="solid", highlightbackground=GOLD, highlightthickness=1)
    ...
    def _build_main_ui(self):
        ...
        tk.Label(topbar, text="CLUE", bg=BG_HEADER, fg=GOLD, font=("Georgia", 20, "bold")).pack(side="left", padx=20)
        tk.Label(topbar, text="Case File", bg=BG_HEADER, fg=TEXT_DIM, font=("Georgia", 10, "italic")).pack(side="left", padx=(0, 12))

        self.turn_label = tk.Label(topbar, text="", bg=BG_HEADER, fg=TEXT_MAIN, font=("Georgia", 11, "italic"))
        self.turn_label.pack(side="left")

        tk.Checkbutton(topbar, text="Show Bot Hands", variable=self.show_bot_cards, command=self._update_all, bg=BG_HEADER, fg=TEXT_MAIN, activebackground=BG_HEADER, activeforeground=GOLD, selectcolor=BG_PANEL, font=("Georgia", 9)).pack(side="right", padx=(0, 12))
        tk.Button(topbar, text="New Game", bg=GREY, fg=TEXT_MAIN, relief="flat", font=("Georgia", 10), padx=10, command=self._new_game).pack(side="right", padx=16, pady=8)
    ...
    def _draw_board(self):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                canvas.create_rectangle(x0, y0, x1, y1, fill=style["fill"], outline=GOLD if is_human_here else style["accent"], width=2 if is_human_here else 1)
                canvas.create_rectangle(x0 + 5, y0 + 5, x1 - 5, y0 + 18, fill=style["accent"], outline="")
                canvas.create_rectangle(x0 + 10, y0 + 28, x1 - 10, y1 - 10, outline=style["accent"], width=1)
                canvas.create_text((x0 + x1) / 2, y0 + 12, text=room.upper(), fill=BG_DARK, font=("Georgia", 8, "bold"))
                ...
                for index, player_name in enumerate(players):
                    ...
                    canvas.create_text(cx, cy + 16, text=label, fill=fill, font=("Georgia", 6, "bold"))
    ...
    def _build_action_panel(self, parent):
        ...
        self.action_label = tk.Label(self.action_frame, text="", bg=BG_PANEL, fg=TEXT_MAIN, font=("Georgia", 10, "italic"), wraplength=700, justify="left")
    ...
    def _build_log_panel(self, parent):
        ...
        frame.pack(fill="both", expand=True, pady=(0, 8))
        self.log_text = tk.Text(frame, bg=BG_CANVAS, fg=TEXT_MAIN, font=("Courier New", 9), state="disabled", wrap="word", relief="flat", padx=8, pady=8, height=18)
    ...
    def _update_hand(self):
        ...
        tk.Label(self.hand_frame, text="Your Cards", bg=BG_PANEL, fg=GOLD, font=("Georgia", 9, "bold")).pack(anchor="w")
        ...
        for card in self.game.get_human_cards():
            ...
            tile = tk.Frame(cards_row, bg=palette["bg"], padx=8, pady=4, highlightbackground=BORDER, highlightthickness=1)
            tile.pack(side="left", padx=4)
            tk.Label(tile, text=card, bg=palette["bg"], fg=palette["fg"], font=("Georgia", 9, "bold")).pack()
            tk.Label(tile, text=palette["label"], bg=palette["bg"], fg=TEXT_DIM, font=("Georgia", 7)).pack()

        if self.last_shown_card:
            panel = tk.Frame(self.reveal_frame, bg=BG_PANEL_ALT, padx=10, pady=8, highlightbackground=GOLD, highlightthickness=1)
            panel.pack(fill="x")
            tk.Label(panel, text="Latest Reveal", bg=BG_PANEL_ALT, fg=GOLD, font=("Georgia", 8, "bold")).pack(anchor="w")
            tk.Label(panel, text=f"{self.last_shown_card['shower']} showed you {self.last_shown_card['card']}", bg=BG_PANEL_ALT, fg=INK, font=("Georgia", 10, "bold")).pack(anchor="w", pady=(2, 0))

        if self.show_bot_cards.get():
            tk.Label(self.bot_cards_frame, text="Bot Hands", bg=BG_PANEL, fg=GOLD, font=("Georgia", 9, "bold")).pack(anchor="w")
            ...
            for name in self.game.player_names:
                ...
                tk.Label(row, text=name, bg=BG_PANEL, fg=TEXT_MAIN, font=("Georgia", 9, "bold"), width=10, anchor="w").pack(side="left")
                cards_holder = tk.Frame(row, bg=BG_PANEL)
                cards_holder.pack(side="left", fill="x")
                for card in all_cards.get(name, []):
                    palette = CARD_PALETTE[CARD_TYPE[card]]
                    tk.Label(cards_holder, text=card, bg=palette["bg"], fg=palette["fg"], font=("Georgia", 7, "bold"), padx=6, pady=2).pack(side="left", padx=2, pady=1)
    ...
    def _start_turn(self):
        ...
        if self.game.is_human_turn():
            ...
            self.action_label.configure(text=f"{self.game.current_player_name} is considering the case.")
    ...
    def _human_turn(self):
        ...
        tk.Button(self.controls_frame, text="Make Suggestion", bg=GOLD, fg=BG_DARK, font=("Georgia", 11, "bold"), relief="flat", padx=12, pady=6, command=self._suggestion_dialog).pack(side="left", padx=6)
        tk.Button(self.controls_frame, text="Make Accusation", bg=RED, fg=TEXT_MAIN, font=("Georgia", 11, "bold"), relief="flat", padx=12, pady=6, command=self._accusation_dialog).pack(side="left", padx=6)
        tk.Button(self.controls_frame, text="Move Room", bg=BG_CARD, fg=TEXT_MAIN, font=("Georgia", 11), relief="flat", padx=12, pady=6, command=self._move_dialog).pack(side="left", padx=6)
    ...
    def _move_dialog(self):
        ...
        tk.Label(dialog, text="Choose a room to move to", bg=BG_DARK, fg=TEXT_MAIN, font=("Georgia", 12)).pack(pady=12, padx=20)
        value = tk.StringVar(value=self.game.get_human_room())
        for room in ROOMS:
            tk.Radiobutton(dialog, text=room, variable=value, value=room, bg=BG_DARK, fg=TEXT_MAIN, selectcolor=BG_CARD, activebackground=BG_DARK, font=("Georgia", 11)).pack(anchor="w", padx=26)
        ...
        tk.Button(dialog, text="Move", bg=GOLD, fg=BG_DARK, font=("Georgia", 11, "bold"), relief="flat", padx=16, pady=6, command=confirm).pack(pady=14)
    ...
    def _suggestion_dialog(self):
        ...
        room = self.game.get_human_room()
        tk.Label(dialog, text=f"Suggestion in {room}", bg=BG_DARK, fg=GOLD, font=("Georgia", 13, "bold")).pack(pady=(16, 6), padx=20)
        form = tk.Frame(dialog, bg=BG_DARK)
        form.pack(padx=20, pady=10)

        tk.Label(form, text="Suspect", bg=BG_DARK, fg=TEXT_MAIN, font=("Georgia", 11)).grid(row=0, column=0, sticky="w", pady=4)
        suspect_var = tk.StringVar(value=SUSPECTS[0])
        ttk.Combobox(form, textvariable=suspect_var, values=SUSPECTS, state="readonly", width=20).grid(row=0, column=1, padx=8)

        tk.Label(form, text="Weapon", bg=BG_DARK, fg=TEXT_MAIN, font=("Georgia", 11)).grid(row=1, column=0, sticky="w", pady=4)
        weapon_var = tk.StringVar(value=WEAPONS[0])
        ttk.Combobox(form, textvariable=weapon_var, values=WEAPONS, state="readonly", width=20).grid(row=1, column=1, padx=8)

        def confirm():
            dialog.destroy()
            self.action_label.configure(text="Investigating suggestion...")
            result = self.game.make_suggestion(self.game.human_name, suspect_var.get(), weapon_var.get(), room)
            self._update_all()
            self._handle_suggestion_result(result)

        tk.Button(dialog, text="Suggest", bg=GOLD, fg=BG_DARK, font=("Georgia", 12, "bold"), relief="flat", padx=16, pady=7, command=confirm).pack(pady=14)
    ...
    def _handle_suggestion_result(self, result):
        if result.get("type") == "shown":
            ...
            self.action_label.configure(text="Nobody could refute the suggestion. The envelope just got more interesting.")
            self._update_all()
            self._after(1800, self._human_turn_done)
        else:
    ...
    def _show_reveal_popup(self, shower, card):
        ...
        tk.Label(inner, text="CARD REVEALED", bg=BG_PANEL_ALT, fg=GOLD, font=("Georgia", 9, "bold")).pack()
        tk.Label(inner, text=card, bg=BG_PANEL_ALT, fg=INK, font=("Georgia", 15, "bold")).pack(pady=(4, 0))
        tk.Label(inner, text=f"shown by {shower}", bg=BG_PANEL_ALT, fg=TEXT_DIM, font=("Georgia", 8, "italic")).pack()
    ...
    def _ask_human_to_show(self, event):
        ...
        tk.Label(dialog, text=f"{event['asker']} made a suggestion.", bg=BG_DARK, fg=TEXT_MAIN, font=("Georgia", 11)).pack(pady=(16, 4), padx=20)
        tk.Label(dialog, text="Choose one card to show", bg=BG_DARK, fg=GOLD, font=("Georgia", 12, "bold")).pack()

        value = tk.StringVar(value=event["cards_can_show"][0])
        for card in event["cards_can_show"]:
            tk.Radiobutton(dialog, text=card, variable=value, value=card, bg=BG_DARK, fg=TEXT_MAIN, selectcolor=BG_CARD, activebackground=BG_DARK, font=("Georgia", 11)).pack(anchor="w", padx=28, pady=2)
        ...
        tk.Button(dialog, text="Show Card", bg=GOLD, fg=BG_DARK, font=("Georgia", 12, "bold"), relief="flat", padx=16, pady=7, command=confirm).pack(pady=14)
    ...
    def _accusation_dialog(self):
        ...
        tk.Label(dialog, text="Final Accusation", bg=BG_DARK, fg=RED, font=("Georgia", 16, "bold")).pack(pady=(16, 4), padx=20)
        tk.Label(dialog, text="A wrong accusation means elimination.", bg=BG_DARK, fg=TEXT_DIM, font=("Georgia", 9, "italic")).pack()
        ...
        for row, (label, var, values) in enumerate([("Suspect", suspect_var, SUSPECTS), ("Weapon", weapon_var, WEAPONS), ("Room", room_var, ROOMS)]):
            tk.Label(form, text=label, bg=BG_DARK, fg=TEXT_MAIN, font=("Georgia", 11)).grid(row=row, column=0, sticky="w", pady=4)
            ttk.Combobox(form, textvariable=var, values=values, state="readonly", width=20).grid(row=row, column=1, padx=8)

        def confirm():
            dialog.destroy()
            result = self.game.make_accusation(self.game.human_name, suspect_var.get(), weapon_var.get(), room_var.get())
            self._update_all()
        ...
        tk.Button(dialog, text="Accuse", bg=RED, fg=TEXT_MAIN, font=("Georgia", 12, "bold"), relief="flat", padx=16, pady=7, command=confirm).pack(pady=14)
    ...
    def _show_game_over(self):
        ...
        tk.Label(summary, text=f"Solution: {solution['suspect']} | {solution['weapon']} | {solution['room']}", bg=BG_PANEL, fg=INK, font=("Georgia", 11, "bold")).pack(anchor="w", pady=(0, 8))
        tk.Label(summary, text="Final Hands", bg=BG_PANEL, fg=GOLD, font=("Georgia", 10, "bold")).pack(anchor="w")

        all_cards = self.game.get_player_cards()
        for name in self.game.player_names:
            block = tk.Frame(summary, bg=BG_PANEL_ALT, padx=8, pady=6, highlightbackground=BORDER, highlightthickness=1)
            block.pack(fill="x", pady=3)
            tk.Label(block, text=name, bg=BG_PANEL_ALT, fg=TEXT_MAIN, font=("Georgia", 9, "bold")).pack(anchor="w")
            row = tk.Frame(block, bg=BG_PANEL_ALT)
            row.pack(anchor="w", fill="x", pady=(4, 0))
            for card in all_cards.get(name, []):
                palette = CARD_PALETTE[CARD_TYPE[card]]
                tk.Label(row, text=card, bg=palette["bg"], fg=palette["fg"], font=("Georgia", 7, "bold"), padx=6, pady=2).pack(side="left", padx=2, pady=1)

        tk.Button(self.controls_frame, text="Play Again", bg=GOLD, fg=BG_DARK, font=("Georgia", 12, "bold"), relief="flat", padx=14, pady=6, command=self._new_game).pack()
